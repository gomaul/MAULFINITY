...
describe('EventBus', () => {
  ...
  describe('Event Subscription', () => {
    ...
    it('should support multiple listeners for the same event', () => {
      ...
      eventBus.on('gift', callback2)

      expect(eventBus.getListenerCount('gift')).toBe(2)
    })
  ...
  describe('Once Subscribers', () => {
    ...
    it('should remove once listener after firing', async () => {
      const callback = vi.fn()
      eventBus.once('gift', callback)
      ...
      expect(eventBus.getListenerCount('gift')).toBe(0)
    })
  })
  ...
  describe('Event History', () => {
    ...
    it('should support limit parameter in getHistory', async () => {
      for (let i = 0; i < 5; i++) {
        await eventBus.emit({
          id: `test-${i}`,
          type: 'gift',
      ...
      const limitedHistory = eventBus.getHistory(3)
  ...
  describe('Statistics', () => {
    ...
    it('should track last event timestamp', async () => {
      ...
      const stats = eventBus.getStats()
      expect(stats.lastEventTimestamp).toBe(timestamp)
    })
  })
  ...
  describe('Listener Removal', () => {
    ...
    it('should remove all listeners for a type with removeAllListeners()', () => {
      eventBus.on('gift', vi.fn())
      eventBus.on('gift', vi.fn())
      eventBus.on('comment', vi.fn())

      eventBus.removeAllListeners('gift')

      expect(eventBus.hasListeners('gift')).toBe(false)
      expect(eventBus.hasListeners('comment')).toBe(true)
    })

    it('should remove all listeners with removeAllListeners() without args', () => {
      eventBus.on('gift', vi.fn())
      eventBus.on('comment', vi.fn())
      ...
      expect(eventBus.hasListeners('gift')).toBe(false)
      expect(eventBus.hasListeners('comment')).toBe(false)
    })
  })
